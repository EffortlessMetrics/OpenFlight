...
fn validate_workspace_consistency() -> ValidationResult {
    ...
    println!("Found {} workspace members to validate", workspace_members.len());
    ...
    for line in lines {
        ...
        if trimmed.starts_with("members = [") {
            ...
            if trimmed.ends_with("]") {
                let members_line = trimmed.strip_prefix("members = [").unwrap()
                    .strip_suffix("]").unwrap();
        ...
        if in_members {
            ...
            // Parse member entry
            if let Some(member) = trimmed.strip_prefix('"').and_then(|s| s.strip_suffix("\",")) {
