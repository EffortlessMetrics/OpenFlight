...
fn main() {
    ...
    println!("  - Valid: {}", fixtures_dir.join("minimal/specs/spec_ledger.yaml").display());
    println!("  - Invalid: {}", fixtures_dir.join("invalid/schema_errors/").display());
    
    println!("\nSchemas available:");
    println!("  - {}", schemas_dir.join("spec_ledger.schema.json").display());
    println!("  - {}", schemas_dir.join("front_matter.schema.json").display());
    println!("  - {}", schemas_dir.join("invariants.schema.json").display());
    
    println!("\nExample error format:");
    println!("[ERROR] INF-SCHEMA-100: Missing required field 'status'");
    println!("  File: specs/spec_ledger.yaml:15:3");
    println!("  Expected: status field with value 'draft', 'implemented', 'tested', or 'deprecated'");
