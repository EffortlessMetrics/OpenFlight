...
use crate::gherkin::{parse_feature_files, GherkinScenario};
...
pub fn generate_feature_status(
    ledger: &SpecLedger,
    ...
    for scenario in scenarios {
        for ac_tag in scenario.ac_tags() {
            ac_to_scenarios
                .entry(ac_tag)
                .or_default()
                .push(format!("{}:{}", scenario.file_path.display(), scenario.line_number));
        }
    }
    ...
    // Generate table
    output.push_str("| REQ ID | AC ID | Description | Gherkin (file:line) | Tests (count) | Status |\n");
    output.push_str("|--------|-------|-------------|---------------------|---------------|--------|\n");
...
pub fn run_ac_status() -> Result<()> {
    ...
    let ledger: SpecLedger = serde_yaml::from_str(&ledger_content)
        .context("Failed to parse spec ledger YAML")?;

    println!("  ✓ Loaded spec ledger with {} requirements", ledger.requirements.len());

    // Parse Gherkin features
    let features_dir = Path::new("specs/features");
    let scenarios = parse_feature_files(features_dir)
    ...
    full_report.push_str(&format!("  Generated at: {}\n", Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
    full_report.push_str(&format!("  Git commit: {}\n", get_git_commit().unwrap_or_else(|_| "unknown".to_string())));
    ...
    std::fs::write(output_path, full_report)
        .context("Failed to write feature status report")?;

    println!("  ✓ Generated feature status report at {}", output_path.display());
