...
fn generate_feature_status_with_metrics(
    ...
    let generated_at = get_git_commit_timestamp_utc()
...
/// Get the current git commit timestamp normalized to UTC.
///
/// # Returns
///
/// Returns a stable timestamp string based on `HEAD` commit time.
fn get_git_commit_timestamp_utc() -> Result<String> {
    let output = Command::new("git")
    ...
    let raw = String::from_utf8(output.stdout)
        ...
        .trim()
        .to_string();
    let parsed = DateTime::parse_from_rfc3339(&raw).context("Failed to parse git timestamp")?;
    Ok(parsed
        .with_timezone(&Utc)
