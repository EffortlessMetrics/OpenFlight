...
**Issue**: Discrepancy between clippy.toml (1.75.0) and workspace Cargo.toml (1.93)

**Resolution**: Updated clippy.toml to `msrv = "1.93"` to match workspace MSRV
...
**CI Impact**: All lint jobs now pin to `dtolnay/rust-toolchain@1.93` to prevent drift from automatic toolchain updates.
...
1. **clippy-core**: Dedicated job for flight-core lint validation
   ...
   - Path filters: `crates/flight-core/**`, `Cargo.toml`, `clippy.toml`
    - Pinned toolchain: `dtolnay/rust-toolchain@1.93`
