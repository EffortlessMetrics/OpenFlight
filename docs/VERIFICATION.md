---
doc_id: DOC-VERIFICATION
kind: reference
area: ci
status: active
links:
  requirements: ["INF-REQ-5", "INF-REQ-9"]
  tasks: []
  adrs: []
---
...
`OpenFlight` has three verification surfaces:
...
### Hardware and backend receipts

OpenFlight is a hardware/backend repository. Hardware receipt badges should only be added when generated from current receipts for the specific backend or device lane.

A hardware badge is not a blanket support promise. It is a generated proof surface for the receipt it names.

### Release

The release badge shows the latest GitHub release. GitHub releases are the public version surface for this repository; crates.io downloads and docs.rs remain registry and documentation surfaces for published crates.
...
Pull requests run advisory `ripr` evidence, review guidance, and targeted follow-up checks when routing rules require them.
