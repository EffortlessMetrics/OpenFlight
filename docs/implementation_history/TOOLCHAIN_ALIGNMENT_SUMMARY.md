...
- `rust-version = "1.93"` (single source of truth)

**clippy.toml:**
- `msrv = "1.93"` (already aligned)

**Status:** Both files are already aligned to MSRV 1.93. No changes needed.

### 2. CI Toolchain Configuration

**Current State:**
- Main test job (`.github/workflows/ci.yml` line 30): Uses `dtolnay/rust-toolchain@master` with matrix `[stable, 1.93]`
- IPC bench job (line 177): Uses `dtolnay/rust-toolchain@stable`
- Other jobs: Mix of `@stable` and `@nightly`

**Note:** Task 12 will update CI workflows to pin toolchain to `dtolnay/rust-toolchain@1.93` for all lint jobs.
