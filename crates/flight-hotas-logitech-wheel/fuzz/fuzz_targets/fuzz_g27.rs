...
//! Fuzz target for the Logitech G27/G25 HID report parser.
//!
//! Exercises `parse_g27` with arbitrary byte slices to ensure no panics or
//! out-of-range axis values on any input.
//!
//! Run with: `cargo +nightly fuzz run fuzz_g27`

#![no_main]

use flight_hotas_logitech_wheel::{WheelError, normalize_pedal, normalize_wheel, parse_g27};
use libfuzzer_sys::fuzz_target;

fuzz_target!(|data: &[u8]| {
    match parse_g27(data) {
        Ok(state) => {
