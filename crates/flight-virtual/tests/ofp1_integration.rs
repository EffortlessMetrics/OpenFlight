...
//! Comprehensive integration tests for OFP-1 protocol implementation
//!
...
//!
//! These tests were relocated from flight-hid to break the cyclic dependency
...
fn test_complete_ofp1_handshake() {
    ...
    let protocol_version = capabilities.protocol_version;
    let max_torque_mnm = capabilities.max_torque_mnm;
    let min_period_us = capabilities.min_period_us;
    assert_eq!(protocol_version, OFP1_VERSION);
    assert_eq!(max_torque_mnm, 20000);
    assert_eq!(min_period_us, 500);
...
fn test_torque_conversion_accuracy() {
    let max_torque = 15.0;
    let test_values = [-15.0, -10.0, -5.0, -1.0, 0.0, 1.0, 5.0, 10.0, 15.0];
...
fn test_complete_integration_scenario() {
    ...
    for &torque_nm in &torque_sequence {
        let torque_protocol =
            utils::torque_nm_to_protocol(torque_nm, negotiation_result.max_torque_nm);
        ...
        if let Some(health) = emulator.read_health_status().unwrap() {
            ...
            let actual_torque =
                utils::torque_protocol_to_nm(current_torque, negotiation_result.max_torque_nm);
            let error = (actual_torque - torque_nm).abs();
            assert!(
                error < 0.1,
                "Torque tracking error: expected {}, got {} (error: {})",
                torque_nm,
                actual_torque,
                error
            );
