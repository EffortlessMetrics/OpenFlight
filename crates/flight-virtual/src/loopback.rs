...
impl LoopbackHid {
    ...
    pub fn send_input_report(&self, report: HidReport) -> bool {
        ...
        if self.input_ring.try_push(report) {
            self.stats.input_reports_sent.fetch_add(1, Ordering::Relaxed);
            self.stats.bytes_transferred.fetch_add(size, Ordering::Relaxed);
            true
        } else {
    ...
    pub fn receive_input_report(&self) -> Option<HidReport> {
        if let Some(report) = self.input_ring.try_pop() {
            ...
            if latency_us > current_max {
                self.stats.max_latency_us.store(latency_us, Ordering::Relaxed);
            }
    ...
    pub fn receive_output_report(&self) -> Option<HidReport> {
        if let Some(report) = self.output_ring.try_pop() {
            self.stats.output_reports_received.fetch_add(1, Ordering::Relaxed);
            Some(report)
        } else {
            None
        }
    ...
    pub fn reset_stats(&self) {
        ...
        self.stats.output_reports_received.store(0, Ordering::Relaxed);
