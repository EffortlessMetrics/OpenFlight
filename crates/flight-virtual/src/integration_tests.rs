...
fn test_multi_device_scheduler() {
    ...
    for (i, device) in devices.iter().enumerate() {
        let device_stats = device.get_stats();
        assert!(
            device_stats.input_reports > 40,
            "Device {} reports: {}",
            i,
            device_stats.input_reports
        );
    }
}
