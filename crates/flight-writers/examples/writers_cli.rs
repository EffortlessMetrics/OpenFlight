...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
            ...
            if result.success {
                ...
                if !result.mismatched_files.is_empty() {
                    ...
                    for mismatch in &result.mismatched_files {
                        println!("   ❌ {}: {:?}", mismatch.file.display(), mismatch.mismatch_type);
                    }
                }
            }
        }
        ...
        Commands::Test { sim } => {
            ...
            println!("\n📊 Test Results:");
            println!("   Overall: {}", if result.success { "✅ PASS" } else { "❌ FAIL" });
            ...
            println!("   Versions: {} ({})", result.coverage.versions.len(), 
                   result.coverage.versions.join(", "));
            println!("   Areas: {} ({})", result.coverage.areas.len(),
            ...
            if backups.is_empty() {
                ...
                for backup in &backups {
                    let timestamp = chrono::DateTime::from_timestamp(backup.timestamp as i64, 0)
                        .unwrap_or_default();
                    println!("   📦 {} ({})", backup.id, timestamp.format("%Y-%m-%d %H:%M:%S"));
        ...
        Commands::Coverage { sim } => {
            ...
            println!("├─────────────────────┼─────────────────────┤");
            println!("│ Overall Coverage    │ {:.1}%              │", result.coverage.coverage_percent);
            println!("│ Versions Covered    │ {}                  │", result.coverage.versions.len());
            println!("│ Areas Covered       │ {}                  │", result.coverage.areas.len());
            println!("│ Test Cases          │ {}                  │", result.test_cases.len());
            println!("│ Passing Tests       │ {}                  │", result.test_cases.iter().filter(|tc| tc.success).count());
            println!("└─────────────────────┴─────────────────────┘");
...
fn parse_simulator_type(sim: &str) -> Result<SimulatorType, Box<dyn std::error::Error>> {
    match sim.to_lowercase().as_str() {
        ...
        _ => Err(format!("Unknown simulator type: {}. Use 'msfs', 'xplane', or 'dcs'", sim).into()),
    }
}