...
use flight_axis::{
    AllocationGuard, AxisFrame, MixerConfig, MixerInput, MixerNode, Node, PipelineBuilder,
};
...
fn test_mixer_config_validation() {
    // Valid configuration
    let valid_config = MixerConfig::new("test").add_scaled_input("input1", 1.0);
    ...
    let config_with_extreme_values =
        MixerConfig::new("test").add_input(MixerInput::new("test", 15.0, 1.0)); // Scale will be clamped to 10.0
...
fn test_mixer_node_creation() {
    let config = MixerConfig::new("test").add_scaled_input("input1", 0.5);
...
fn test_mixer_node_trait_implementation() {
    ...
    assert_eq!(
        mixer.state_size(),
        std::mem::size_of::<flight_axis::MixerState>()
    );
}
...
fn test_mixer_deterministic_processing() {
    ...
    for &(inputs, description) in &test_cases {
        ...
        assert_eq!(
            output1, output2,
            "Non-deterministic output for {}: inputs={:?}",
            description, inputs
        );
    }
}
...
fn test_mixer_mathematical_properties() {
    ...
    assert!(
        (output_scaled_x - 2.0 * output_x).abs() < 1e-6,
    ...
    assert!(
        (output_combined - (output_x + output_y)).abs() < 1e-6,
...
fn test_mixer_performance_characteristics() {
    ...
    assert!(
        avg_time_per_call < Duration::from_nanos(1000),
        "Mixer processing too slow: {:?} per call",
        avg_time_per_call
    );
}
...
fn test_mixer_in_pipeline_compilation() {
    ...
    let result = PipelineBuilder::new().deadzone(0.05).mixer(config);
...
fn test_mixer_unit_handling_validation() {
    ...
    for &(inputs, _expected_unclamped, expected_clamped) in &boundary_cases {
        ...
        assert!(
            (output - expected_clamped).abs() < 1e-6,
...
fn test_mixer_zero_allocation_integration() {
...

    {
        ...
        assert!(
            !AllocationGuard::allocations_detected(),
