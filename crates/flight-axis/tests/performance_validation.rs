...
use flight_axis::{AxisEngine, AxisFrame, EngineConfig, PipelineBuilder};
...
fn test_rt_performance_requirements() {
    ...
    assert!(
        p99_time < std::time::Duration::from_micros(500),
        "P99 processing time {}μs exceeds 500μs requirement",
        p99_time.as_micros()
    );

    assert!(
        avg_time < std::time::Duration::from_micros(100),
...
fn test_jitter_requirements() {
    ...
    assert!(
        p99_jitter < std::time::Duration::from_micros(500),
...
fn test_atomic_swap_performance() {
    ...
    assert_eq!(
        engine.swap_ack_count(),
        initial_ack + 1,
        "Swap not acknowledged"
    );

    // Swap should be very fast (< 100μs)
    assert!(
        swap_time < std::time::Duration::from_micros(100),
