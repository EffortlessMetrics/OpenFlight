...
use flight_axis::{AxisFrame, DetentEvent, DetentNode, DetentRole, DetentState, DetentZone, Node};
...
unsafe fn process_frame_soa(node: &DetentNode, frame: &mut AxisFrame, state: &mut DetentState) {
...
fn test_single_detent_entry_exit() {
    let zones = vec![DetentZone::new(0.0, 0.1, 0.05, DetentRole::Idle)];
    ...
    unsafe {
        process_frame_soa(&node, &mut frame, &mut state);
    }
...
fn test_no_snap_detent() {
    let zones = vec![DetentZone::no_snap(0.0, 0.1, 0.02, DetentRole::Custom(1))];
    let (node, receiver) = create_test_detent_node(zones);
    let mut state = DetentState {
        active_detent_idx: u32::MAX,
        last_position: 0.0,
...
fn test_sweep_single_transitions() {
    ...
    for (_, positions) in transition_positions {
        assert_eq!(
            positions.len(),
            1,
            "Multiple transitions of same type detected"
        );
    }
}
