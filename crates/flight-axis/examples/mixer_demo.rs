...
use flight_axis::{AxisEngine, AxisFrame, EngineConfig, MixerConfig, MixerNode, PipelineBuilder};
...
fn demonstrate_helicopter_anti_torque() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &(inputs, description) in &scenarios {
        ...
        println!(
            "  {} -> Collective: {:.2}, Pedals: {:.2} -> Output: {:.3} ({})",
...
fn demonstrate_custom_mixer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Collective: {:.1}, Pedals: {:.1}, Airspeed: {:.1}, Engine Torque: {:.1}",
    ...
    println!(
        "    Total: {:.3}",
        collective_contrib + pedals_contrib + airspeed_contrib + engine_contrib
    );
    println!();
...
fn demonstrate_mixer_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let engine = AxisEngine::with_config(
        "mixer_demo".to_string(),
        EngineConfig {
