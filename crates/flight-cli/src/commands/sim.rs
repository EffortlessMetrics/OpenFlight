...
use flight_ipc::{
    DetectCurveConflictsRequest, OneClickResolveRequest, ResolutionAction, ResolutionType,
    ResolveCurveConflictRequest,
};
...
async fn resolve_conflict(
    ...
    if let Some(ref resolution_result) = response.result {
        result["applied_resolution"] = json!(resolution_type_to_string(
            resolution_result.applied_resolution()
        ));
...
async fn one_click_resolve(
    ...
    if let Some(ref one_click_result) = response.result {
        result["resolution_type"] = json!(resolution_type_to_string(
            one_click_result.resolution_type()
        ));
        ...
        if verbose && let Some(ref metrics) = one_click_result.metrics {
            ...
            let steps: Vec<Value> = one_click_result
                .steps_performed
                .iter()
                .map(|step| {
                    json!({
