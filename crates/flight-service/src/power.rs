...
impl PowerChecker {
    ...
    pub async fn check_power_configuration() -> PowerStatus {
        ...
        let overall_status = if checks.iter().any(|c| c.status == PowerCheckStatus::Critical) {
            PowerCheckStatus::Critical
        } else if checks.iter().any(|c| c.status == PowerCheckStatus::Degraded) {
    ...
    async fn check_usb_selective_suspend(
        ...
        checks.push(PowerCheck {
            ...
            current_value: Some(if usb_suspend_enabled { "Enabled" } else { "Disabled" }.to_string()),
    ...
    async fn check_power_plan(
        ...
        let is_high_performance = current_plan.contains("High performance") || current_plan.contains("Ultimate");
    ...
    async fn check_process_power_throttling(
        ...
        checks.push(PowerCheck {
            ...
            current_value: Some(if throttling_disabled { "Disabled" } else { "Enabled" }.to_string()),
            expected_value: Some("Disabled".to_string()),
        });
        
        if !throttling_disabled {
            remediation_steps.push(RemediationStep {
                description: "Process power throttling will be disabled automatically at runtime".to_string(),
    ...
    async fn check_rtkit_availability(
        ...
        checks.push(PowerCheck {
            ...
            current_value: Some(if rtkit_available { "Available" } else { "Not Available" }.to_string()),
    ...
    async fn check_memlock_limits(
        ...
        if !sufficient {
            remediation_steps.push(RemediationStep {
                ...
                action: "Add 'username hard memlock 65536' to /etc/security/limits.conf".to_string(),
                requires_admin: true,
                priority: 2,
            });
        }
