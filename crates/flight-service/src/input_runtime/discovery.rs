...
pub(super) async fn sync_visible_devices(
    ...
    for info in source.list_devices() {
        ...
        states.insert(
            ...
            DeviceRuntimeState {
                ...
                model,
                handler: new_input_handler(model, axis_mode_hint, config),
                monitor: TFlightHealthMonitor::new(model).with_legacy_pid(is_legacy),
...
fn new_input_handler(
    model: flight_hotas_thrustmaster::TFlightModel,
    axis_mode_hint: AxisMode,
    config: TFlightRuntimeConfig,
) -> TFlightInputHandler {
    // Always start in Unknown so the handler auto-detects every report; the
    // descriptor hint is advisory only (see fix for runtime AxisMode pinning).
    TFlightInputHandler::with_axis_mode(model, AxisMode::Unknown)
...
async fn emit_new_device_guidance(
    health: &HealthStream,
    snapshot_key: &str,
    model: flight_hotas_thrustmaster::TFlightModel,
    axis_mode_hint: AxisMode,
