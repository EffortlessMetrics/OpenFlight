...
async fn poll_once(
    ...
    let active_paths = discovery::sync_visible_devices(source, health, states, config).await;
    discovery::remove_disconnected_devices(health, snapshots, states, &active_paths).await;
    report_processing::process_available_reports(source, health, snapshots, states).await;
}
