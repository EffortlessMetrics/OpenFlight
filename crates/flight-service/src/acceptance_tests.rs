...
async fn test_service_lifecycle() {
    ...
    assert_eq!(state, ServiceState::Running, "Service should be in running state");
...
async fn test_safe_mode_service() {
    ...
    let result = service.start().await;
    assert!(result.is_ok(), "Safe mode service should start successfully");
    
    // Verify safe mode state
    let state = service.get_state().await;
    assert_eq!(state, ServiceState::SafeMode, "Service should be in safe mode");
    ...
    assert!(result.is_ok(), "Safe mode service should shutdown successfully");
}
...
async fn test_power_configuration() {
    ...
    let result = service.start().await;
    assert!(result.is_ok(), "Service should start even with power checks");
    
    // Get power status
    let power_status = service.get_power_status().await;
    assert!(power_status.is_some(), "Should have power status when enabled");
    
    let status = power_status.unwrap();
    assert!(!status.checks.is_empty(), "Should have performed power checks");
...
async fn test_shutdown_signal() {
    ...
    assert!(shutdown_rx.is_some(), "Should be able to subscribe to shutdown");
...
async fn test_comprehensive_service_functionality() {
    ...
    assert!(start_result.is_ok(), "Comprehensive service should start successfully");
...
async fn test_service_resilience() {
    ...
    // Start service
    let result = service.start().await;
    assert!(result.is_ok(), "Service should start even if some components fail");
