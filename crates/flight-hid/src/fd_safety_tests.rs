...
mod tests {
    ...
    fn test_typed_fd_usage() {
        ...
        let _ = (example_function_with_owned_fd, example_function_with_borrowed_fd, example_function_with_as_fd);
    }
    ...
    fn test_no_raw_fd_in_public_apis() {
        ...
        use crate::{HidAdapter, HidDeviceInfo, EndpointId, HidOperationResult};
    ...
    fn test_no_raw_fd_in_debug_output() {
        use crate::{HidDeviceInfo, EndpointId, EndpointType, HidOperationResult};
        ...
        let operation_result = HidOperationResult::Success { bytes_transferred: 10 };
        ...
        assert!(!device_debug.contains("RawFd"), "HidDeviceInfo debug output contains RawFd");
        assert!(!endpoint_debug.contains("RawFd"), "EndpointId debug output contains RawFd");
        assert!(!result_debug.contains("RawFd"), "HidOperationResult debug output contains RawFd");
    }
}
