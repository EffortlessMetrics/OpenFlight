...
pub enum Ofp1Error {
    #[error(
        "Protocol version mismatch: device {device_version:04X}, expected {expected_version:04X}"
    )]
    VersionMismatch {
        device_version: u16,
...
impl Ofp1Negotiator {
    ...
    pub fn negotiate(&self, capabilities: &CapabilitiesReport) -> Result<Ofp1NegotiationResult> {
        ...
        let supports_high_torque =
            max_torque_nm >= 5.0 && capability_flags.has_flag(CapabilityFlags::PHYSICAL_INTERLOCK);
        ...
        Ok(Ofp1NegotiationResult {
            ...
            device_serial: String::from_utf8_lossy(&capabilities.serial_number)
                .trim_end_matches('\0')
                .to_string(),
        })
    }
...
impl Ofp1HealthMonitor {
    ...
    pub fn update_health(&mut self, health: HealthStatusReport) -> Result<()> {
        ...
        if status_flags.has_fault() {
            ...
            return Err(Ofp1Error::DeviceFault {
                fault_code: fault_code as u8,
                description,
            });
        }
