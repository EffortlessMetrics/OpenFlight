...
async fn test_update_rollback_cycle() {
    ...
    fs::write(install_dir.join("app.exe"), b"version 1.0.0").await.unwrap();
    ...
    fs::write(install_dir.join("app.exe"), b"version 1.1.0").await.unwrap();
...
async fn test_complete_update_flow() {
    ...
    fs::write(config.install_dir.join("main.exe"), b"original").await.unwrap();
