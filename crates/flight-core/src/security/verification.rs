...
impl SecurityVerifier {
    ...
    pub fn audit_event(&mut self, event: AuditEvent) {
        ...
        if self.audit_log.len() > self.config.max_audit_events {
            self.audit_log.drain(0..self.audit_log.len() - self.config.max_audit_events);
        }
    }
    ...
    async fn verify_ipc_security(&mut self) -> Result<Vec<SecurityCheck>> {
        ...
        checks.push(SecurityCheck {
            ...
            remediation: Some("Configure proper ACLs to restrict IPC access to authorized users only".to_string()),
        });
    ...
    async fn verify_plugin_sandboxing(&mut self) -> Result<Vec<SecurityCheck>> {
        ...
        checks.push(SecurityCheck {
            ...
            description: "WASM plugins are properly sandboxed with no file/network access by default".to_string(),
            details: Some("Verifies WASM runtime denies undeclared capabilities".to_string()),
            severity: SecuritySeverity::Critical,
            remediation: Some("Ensure WASM runtime enforces capability manifests and denies undeclared access".to_string()),
        });
        ...
        checks.push(SecurityCheck {
            ...
            description: "Native plugins run in isolated helper processes with watchdog protection".to_string(),
            details: Some("Verifies native plugins execute in separate address space with SHM communication".to_string()),
            severity: SecuritySeverity::High,
            remediation: Some("Configure native plugins to run in isolated helper processes with proper IPC".to_string()),
        });
    ...
    async fn verify_telemetry_privacy(&mut self) -> Result<Vec<SecurityCheck>> {
        ...
        checks.push(SecurityCheck {
            ...
            remediation: Some("Implement proper data redaction for all support bundle contents".to_string()),
        });
    ...
    async fn verify_network_access(&mut self) -> Result<Vec<SecurityCheck>> {
        ...
        checks.push(SecurityCheck {
            ...
            details: Some("Verifies service doesn't bind to network interfaces by default".to_string()),
            severity: SecuritySeverity::Critical,
            remediation: Some("Ensure no network sockets are bound unless explicitly configured".to_string()),
        });
    ...
    async fn verify_process_isolation(&mut self) -> Result<Vec<SecurityCheck>> {
        ...
        checks.push(SecurityCheck {
            ...
            details: Some("Verifies integration uses only SimConnect/DataRefs/Export.lua".to_string()),
            severity: SecuritySeverity::Critical,
            remediation: Some("Use only approved integration methods (SimConnect, DataRefs, Export.lua)".to_string()),
        });
        ...
        for check in checks {
            if check.status == VerificationStatus::Fail {
                recommendations.push(SecurityRecommendation {
                    ...
                    action_required: matches!(check.severity, SecuritySeverity::Critical | SecuritySeverity::High),
                    remediation_steps: check.remediation.as_ref()
                        .map(|r| vec![r.clone()])
                        .unwrap_or_default(),
                });
            }
