...
pub use verification::{
    SecurityVerifier, SecurityVerificationResult, SecurityCheck, AuditEvent,
    SecurityRecommendation, VerificationStatus, SecurityCategory, SecuritySeverity,
    AuditEventType, RecommendationPriority, VerificationConfig, VerificationError,
};
...
impl SecurityManager {
    ...
    pub fn get_redacted_support_data(&self) -> HashMap<String, serde_json::Value> {
        ...
        if self.telemetry_config.enabled && self.telemetry_config.include_in_support {
            ...
            if self.telemetry_config.collected_data.contains(&TelemetryDataType::Performance) {
                data.insert("performance_summary".to_string(), serde_json::json!({
            ...
            if self.telemetry_config.collected_data.contains(&TelemetryDataType::Errors) {
                data.insert("error_summary".to_string(), serde_json::json!({
        ...
        data.insert("security_config".to_string(), serde_json::json!({
