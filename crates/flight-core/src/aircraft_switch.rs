...
impl Default for PofHysteresisConfig {
    fn default() -> Self {
        ...
        hysteresis_bands.insert("ias".to_string(), HysteresisBand {
        ...
        // Altitude hysteresis
        hysteresis_bands.insert("altitude".to_string(), HysteresisBand {
        ...
        hysteresis_bands.insert("ground_speed".to_string(), HysteresisBand {
...
impl AircraftAutoSwitch {
    ...
    pub async fn start(&self) -> Result<()> {
        let mut rx = self.switch_rx.write().await.take()
            .ok_or_else(|| FlightError::AutoSwitch("Auto-switch already started".to_string()))?;
                ...
                match Self::process_switch_request(
                    ...
                    Ok(result) => {
                        if let Some(result) = result {
                            ...
                            let current_avg = state_guard.metrics.average_switch_time.as_nanos() as f64;
                            let new_avg = (current_avg * (total - 1.0) + switch_time.as_nanos() as f64) / total;
                            state_guard.metrics.average_switch_time = Duration::from_nanos(new_avg as u64);
                            
                            info!(
                                "Aircraft switch completed: {} -> {} in {:?}",
                                result.old_aircraft.map(|a| a.to_string()).unwrap_or_else(|| "None".to_string()),
                                result.new_aircraft,
                                switch_time
                            );
                        }
    ...
    pub async fn on_aircraft_detected(&self, aircraft: DetectedAircraft) -> Result<()> {
        ...
        self.switch_tx.send(SwitchRequest::AircraftDetected(aircraft))
            .map_err(|e| FlightError::AutoSwitch(format!("Failed to send aircraft detection: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn on_telemetry_update(&self, snapshot: TelemetrySnapshot) -> Result<()> {
        if self.config.enable_pof {
            self.switch_tx.send(SwitchRequest::TelemetryUpdate(snapshot))
                .map_err(|e| FlightError::AutoSwitch(format!("Failed to send telemetry update: {}", e)))?;
        }
        Ok(())
    }
    ...
    pub async fn force_switch(&self, aircraft_id: AircraftId) -> Result<()> {
        ...
        self.switch_tx.send(SwitchRequest::ForceSwitch(aircraft_id))
    ...
    pub async fn invalidate_cache(&self, aircraft_id: Option<AircraftId>) -> Result<()> {
        ...
        self.switch_tx.send(SwitchRequest::InvalidateCache(aircraft_id))
            .map_err(|e| FlightError::AutoSwitch(format!("Failed to send cache invalidation: {}", e)))?;
        Ok(())
    }
    ...
    async fn process_switch_request(
        ...
        match request {
            ...
            SwitchRequest::TelemetryUpdate(snapshot) => {
                Self::handle_telemetry_update(snapshot, state, pof_tracker, profile_cache, config).await
            }
    ...
    async fn handle_aircraft_detection(
        ...
        // Check if this is actually a new aircraft
        {
            ...
            if let Some(current) = &state_guard.current_aircraft {
                if current.aircraft_id == aircraft.aircraft_id && current.sim == aircraft.sim {
 ...
 /// Handle telemetry update for PoF tracking
    async fn handle_telemetry_update(
        ...
        if pof_changed {
            ...
            if let Some(current_aircraft) = &state_guard.current_aircraft {
                if let Some(current_profile) = &state_guard.current_profile {
   ...
   /// Load and compile profile for aircraft
    async fn load_and_compile_profile(
        ...
        // Check cache first
        {
            ...
            if let Some(cached) = cache.profiles.get(aircraft_id) {
                if let Some(timestamp) = cache.cache_timestamps.get(aircraft_id) {
        ...
        {
            let mut cache = profile_cache.write().await;
            cache.profiles.insert(aircraft_id.clone(), CachedProfile {
                base_profile: compiled.profile.clone(),
                compiled: compiled.clone(),
                file_path: PathBuf::new(), // TODO: Track actual file path
                last_modified: Instant::now(),
            });
            cache.cache_timestamps.insert(aircraft_id.clone(), Instant::now());
        }
    ...
    async fn load_profile_hierarchy(
        ...
        // Load global profile
        if let Ok(global_profile) = Self::load_profile_from_path(&config.profile_paths[0], "global.json").await {
        ...
        if let Ok(sim_profile) = Self::load_profile_from_path(&config.profile_paths[1], &format!("{}.json", sim_name)).await {
            profiles.push(sim_profile);
        }

        // Load aircraft-specific profile
        let aircraft_filename = format!("{}.json", aircraft_id.icao);
        if let Ok(aircraft_profile) = Self::load_profile_from_path(&config.profile_paths[2], &aircraft_filename).await {
            profiles.push(aircraft_profile);
        }

        if profiles.is_empty() {
            return Err(FlightError::AutoSwitch(format!(
                "No profiles found for aircraft: {}", aircraft_id
            )));
        }
    ...
    async fn load_profile_from_path(base_path: &PathBuf, filename: &str) -> Result<Profile> {
        ...
        let content = tokio::fs::read_to_string(&profile_path).await
            .map_err(|e| FlightError::AutoSwitch(format!("Failed to read profile {}: {}", profile_path.display(), e)))?;
        
        let profile: Profile = serde_json::from_str(&content)
            .map_err(|e| FlightError::AutoSwitch(format!("Failed to parse profile {}: {}", profile_path.display(), e)))?;
  ...
  /// Update PoF with hysteresis logic
    ...
    fn update_parameter_hysteresis(
        ...
        let state = hysteresis_state.entry(param_name.to_string()).or_insert_with(|| {
            HysteresisState {
                current_value,
                in_condition: false,
                last_transition: now,
...
impl std::str::FromStr for PhaseOfFlight {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(FlightError::AutoSwitch(format!("Unknown phase of flight: {}", s))),
        }
    }
}
...
mod tests {
    ...
    async fn test_profile_hierarchy_loading() {
        ...
        fs::write(global_dir.join("global.json"), global_profile).await.unwrap();
        fs::write(sim_dir.join("msfs.json"), sim_profile).await.unwrap();
        fs::write(aircraft_dir.join("C172.json"), aircraft_profile).await.unwrap();
        ...
        let profiles = AircraftAutoSwitch::load_profile_hierarchy(
            &aircraft_id,
            SimId::Msfs,
            &config,
        ).await.unwrap();
    ...
    async fn test_cache_invalidation() {
        ...
        auto_switch.invalidate_cache(Some(aircraft_id.clone())).await.unwrap();
    ...
    fn test_phase_of_flight_string_conversion() {
        assert_eq!("ground".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Ground);
        assert_eq!("taxi".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Taxi);
        assert_eq!("takeoff".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Takeoff);
        assert_eq!("climb".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Climb);
        assert_eq!("cruise".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Cruise);
        assert_eq!("descent".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Descent);
        assert_eq!("approach".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Approach);
        assert_eq!("landing".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::Landing);
        assert_eq!("goaround".parse::<PhaseOfFlight>().unwrap(), PhaseOfFlight::GoAround);
