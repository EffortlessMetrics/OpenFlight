...
impl Profile {
    ...
    pub fn validate_with_capabilities(&self, context: &CapabilityContext) -> Result<()> {
        ...
        if self.schema != PROFILE_SCHEMA_VERSION {
            return Err(FlightError::ProfileValidation(
                format!("Unsupported schema version: {}", self.schema)
            ));
        }
    ...
    fn validate_axis_config(&self, axis_name: &str, config: &AxisConfig, context: &CapabilityContext) -> Result<()> {
        ...
        if let Some(expo) = config.expo {
            if expo < 0.0 || expo > context.limits.max_expo {
                return Err(FlightError::ProfileValidation(
                    format!("axes.{}.expo: Expo must be between 0.0 and {} in {:?} mode", 
        ...
        if let Some(slew_rate) = config.slew_rate {
            if slew_rate < 0.0 || slew_rate > context.limits.max_slew_rate {
                return Err(FlightError::ProfileValidation(
                    format!("axes.{}.slew_rate: Slew rate must be between 0.0 and {} in {:?} mode", 
    ...
    fn validate_curve_monotonic(&self, axis_name: &str, curve: &[CurvePoint]) -> Result<()> {
        if curve.len() < 2 {
            return Err(FlightError::ProfileValidation(
                format!("axes.{}.curve: Curve must have at least 2 points", axis_name)
            ));
        }
        ...
        for i in 1..curve.len() {
            if curve[i].input <= curve[i-1].input {
                return Err(FlightError::ProfileValidation(
                    format!("axes.{}.curve[{}]: Curve input values must be strictly increasing (monotonic)", axis_name, i)
                ));
            }
        }
    ...
    fn validate_detent(&self, axis_name: &str, index: usize, detent: &DetentZone) -> Result<()> {
        if detent.position < -1.0 || detent.position > 1.0 {
            return Err(FlightError::ProfileValidation(
                format!("axes.{}.detents[{}].position: Detent position must be between -1.0 and 1.0", axis_name, index)
            ));
        }
        
        if detent.width <= 0.0 || detent.width > 0.5 {
            return Err(FlightError::ProfileValidation(
                format!("axes.{}.detents[{}].width: Detent width must be between 0.0 and 0.5", axis_name, index)
            ));
        }
    ...
    fn validate_pof_overrides(&self, phase_name: &str, overrides: &PofOverrides, context: &CapabilityContext) -> Result<()> {
        if let Some(axes) = &overrides.axes {
            for (axis_name, config) in axes {
                self.validate_axis_config(&format!("pof_overrides.{}.axes.{}", phase_name, axis_name), config, context)?;
            }
        }
...
impl CapabilityContext {
    ...
    pub fn for_mode(mode: CapabilityMode) -> Self {
        ...
        Self { mode, limits, audit_enabled: false }
    }
}
