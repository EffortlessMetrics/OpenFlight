...
impl RegressionDetector {
    ...
    pub fn check_regression(&self, current: CounterSnapshot) -> RegressionResult {
        ...
        let comparison = baseline.as_ref().map(|b| self.compare_snapshots(&current, b));
    ...
    fn compare_snapshots(&self, current: &CounterSnapshot, baseline: &CounterSnapshot) -> ComparisonStats {
        ComparisonStats {
    ...
    fn compare_metric(&self, current: f64, baseline: f64, threshold: &Threshold) -> MetricComparison {
    ...
    fn generate_alerts(
        ...
        if let Some(comp) = comparison {
            if comp.jitter.is_regression && comp.jitter.relative_change > self.thresholds.jitter_p99_ns.relative_increase {
                alerts.push(Alert {
                    severity: AlertSeverity::Critical,
            ...
            if comp.hid_latency.is_regression && comp.hid_latency.relative_change > self.thresholds.hid_avg_ns.relative_increase {
                alerts.push(Alert {
                    severity: AlertSeverity::Critical,
            ...
            if comp.writer_drops.is_regression && comp.writer_drops.current > 0.0 {
                alerts.push(Alert {
                    ...
                    description: format!(
                        ...
                        comp.writer_drops.current as u64,
                        comp.writer_drops.baseline as u64
                    ),
    ...
    pub fn load_baselines(&mut self, path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn get_baseline_summary(&self) -> Option<BaselineSummary> {
        ...
        let jitter_p99s: Vec<i64> = self.baselines.iter()
            .map(|b| b.jitter.p99_ns)
            .collect();
        
        let hid_avgs: Vec<u64> = self.baselines.iter()
            .map(|b| b.hid.avg_time_ns)
            .collect();
        
        let miss_rates: Vec<f64> = self.baselines.iter()
            .map(|b| b.miss_rate)
            .collect();
...
mod tests {
    ...
    fn test_quality_gate_violations() {
        ...
        let fatal_alerts: Vec<_> = result.alerts.iter()
