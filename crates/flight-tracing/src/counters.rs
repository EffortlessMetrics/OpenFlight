...
impl std::fmt::Display for QualityGateViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{}: expected {}, got {}",
            self.gate, self.threshold, self.actual
        )
    }
}
...
impl CounterSnapshot {
    ...
    pub fn to_kv_pairs(&self) -> Vec<(String, String)> {
        vec![
            ...
            (
                "miss_rate_percent".to_string(),
                format!("{:.4}", self.miss_rate * 100.0),
            ),
            (
                "jitter_p50_us".to_string(),
                format!("{:.1}", self.jitter.p50_ns as f64 / 1_000.0),
            ),
            (
                "jitter_p99_us".to_string(),
                format!("{:.1}", self.jitter.p99_ns as f64 / 1_000.0),
            ),
            ("hid_writes".to_string(), self.total_hid_writes.to_string()),
            (
                "hid_avg_us".to_string(),
                format!("{:.1}", self.hid.avg_time_ns as f64 / 1_000.0),
            ),
            ("writer_drops".to_string(), self.writer_drops.to_string()),
            (
                "session_duration_s".to_string(),
                format!("{:.1}", self.session_duration_ms as f64 / 1_000.0),
            ),
        ]
    }
    ...
    pub fn is_regression(&self, baseline: &CounterSnapshot) -> bool {
        ...
        let jitter_regression = if baseline.jitter.p99_ns != 0 {
            let jitter_increase = (self.jitter.p99_ns - baseline.jitter.p99_ns) as f64
        ...
        let hid_regression = if baseline.hid.avg_time_ns != 0 {
            let hid_increase = (self.hid.avg_time_ns as f64 - baseline.hid.avg_time_ns as f64)
...
mod tests {
    ...
    fn test_kv_export() {
        ...
        assert!(
            kv_pairs
                .iter()
                .any(|(k, v)| k == "jitter_p99_us" && v == "4.5")
        );
        assert!(
            kv_pairs
                .iter()
                .any(|(k, v)| k == "miss_rate_percent" && v == "0.5000")
        );
        assert!(
            kv_pairs
                .iter()
