...
impl TraceProvider for TracepointProvider {
    fn initialize(&mut self) -> Result<(), TraceError> {
        ...
        if !Self::is_available() {
            return Err(TraceError::Platform(
                "Tracepoints not available - /sys/kernel/debug/tracing/trace_marker not found"
                    .to_string(),
            ));
        }
    ...
    fn emit_event(&self, event: &TraceEvent) -> Result<(), TraceError> {
        ...
        let message = match &event.data {
            EventData::TickStart { tick_number } => self.format_tick_start(*tick_number),
            ...
            EventData::Custom { name, data } => self.format_custom(name, data)?,
        };
...
mod tests {
    ...
    fn test_tracepoint_initialization() {
        ...
        match provider.initialize() {
            ...
            Err(e) => {
                println!(
                    "Tracepoint initialization failed (expected if not root): {}",
                    e
                );
            }
        }
