...
use flight_tracing::{
    initialize, shutdown, get_counters, reset_counters,
    trace_tick_start, trace_tick_end, trace_hid_write, trace_deadline_miss,
    TickTracer, HidWriteTracer,
    regression::{RegressionDetector, Thresholds, AlertSeverity},
...
fn run_performance_test(
    ...
    println!("Running {} performance test ({} ticks)...", name, tick_count);
    ...
    for tick in 0..tick_count {
        ...
        if tick % 10 == 0 {
            ...
            let hid_latency = Duration::from_nanos(target_hid_latency_ns + (tick % 50) as u64 * 1000);
        ...
        drop(_tick_tracer);
        trace_tick_end!(tick as u64, tick_duration.as_nanos() as u64, artificial_jitter);
    ...
    println!("Test completed: {} ticks, {:.1}s duration", 
...
fn check_quality_gates(snapshot: &CounterSnapshot) {
    ...
    println!("  Jitter p99: {:.1}μs (limit: 500μs)", snapshot.jitter.p99_ns as f64 / 1_000.0);
    println!("  HID avg latency: {:.1}μs (limit: 300μs)", snapshot.hid.avg_time_ns as f64 / 1_000.0);
    println!("  Miss rate: {:.3}% (limit: 1%)", snapshot.miss_rate * 100.0);
    ...
    if snapshot.jitter.sample_count >= 100 && snapshot.jitter.p99_ns.abs() > 500_000 {
        violations.push(format!("Jitter p99 exceeds 500μs: {:.1}μs", snapshot.jitter.p99_ns as f64 / 1_000.0));
    }
    
    if snapshot.hid.avg_time_ns > 300_000 {
        violations.push(format!("HID latency exceeds 300μs: {:.1}μs", snapshot.hid.avg_time_ns as f64 / 1_000.0));
    }
    
    if snapshot.miss_rate > 0.01 {
        violations.push(format!("Miss rate exceeds 1%: {:.3}%", snapshot.miss_rate * 100.0));
    }
    
    if snapshot.writer_drops > 100 {
        violations.push(format!("Writer drops exceed 100: {}", snapshot.writer_drops));
    }
...
fn demonstrate_regression_detection(
    ...
    println!("  Baseline jitter p99: {:.1}μs", baseline.jitter.p99_ns as f64 / 1_000.0);
    println!("  Current jitter p99: {:.1}μs", current.jitter.p99_ns as f64 / 1_000.0);
    
    if let Some(comp) = &result.comparison {
        println!("  Jitter change: {:.1}% ({:+.1}μs)", 
                 comp.jitter.relative_change * 100.0,
                 comp.jitter.absolute_change / 1_000.0);
        println!("  HID latency change: {:.1}% ({:+.1}μs)",
...
fn export_ci_metrics(snapshot: &CounterSnapshot) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("::set-output name=jitter_p99_us::{:.1}", snapshot.jitter.p99_ns as f64 / 1_000.0);
    println!("::set-output name=hid_avg_us::{:.1}", snapshot.hid.avg_time_ns as f64 / 1_000.0);
    println!("::set-output name=miss_rate_percent::{:.4}", snapshot.miss_rate * 100.0);
