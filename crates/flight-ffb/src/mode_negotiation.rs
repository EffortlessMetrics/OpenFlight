...
impl ModeNegotiator {
    ...
    pub fn negotiate_mode(&self, capabilities: &DeviceCapabilities) -> ModeSelection {
        ...
        let (mode, rationale) = if self.policy.prefer_raw_torque && capabilities.supports_raw_torque
        {
            ...
            if max_rate_hz >= self.policy.min_update_rate_hz {
                (
                    FfbMode::RawTorque,
                    "Raw torque preferred and supported with adequate rate".to_string(),
                )
            } else if capabilities.supports_pid {
                (
                    FfbMode::DirectInput,
                    format!(
                        "Raw torque supported but rate {} Hz < required {} Hz, falling back to DirectInput",
                        max_rate_hz, self.policy.min_update_rate_hz
                    ),
                )
            } else {
                (
                    ...
                    format!(
                        "Raw torque rate {} Hz insufficient, no DirectInput support",
                        max_rate_hz
                    ),
                )
            }
            ...
            (
                FfbMode::DirectInput,
                "DirectInput PID effects supported".to_string(),
            )
        } else {
            (
                FfbMode::TelemetrySynth,
                "No hardware FFB support, using telemetry synthesis".to_string(),
            )
        };
    ...
    fn validate_capabilities(&self, capabilities: &DeviceCapabilities) -> Result<(), String> {
        ...
        if capabilities.max_torque_nm > 50.0 {
            return Err(format!(
                "Excessive max torque: {} Nm > 50 Nm safety limit",
                capabilities.max_torque_nm
            ));
        }
        ...
        if capabilities.supports_raw_torque && capabilities.min_period_us > 0 {
            ...
            if max_rate_hz < 10 {
                return Err(format!(
                    "Raw torque update rate {} Hz too low (< 10 Hz)",
                    max_rate_hz
                ));
            }
        }
    ...
    fn check_high_torque_support(&self, capabilities: &DeviceCapabilities, mode: &FfbMode) -> bool {
        ...
        if self.policy.require_health_stream_for_high_torque
            && !capabilities.has_health_stream
    ...
    fn test_mode_selection_matrix() {
        ...
        for test_case in test_matrix {
            ...
            assert_eq!(
                ...
                "Mode mismatch for test case: {}",
                test_case.name
            );

            assert_eq!(
                selection.supports_high_torque, test_case.expected_high_torque,
                "High torque support mismatch for test case: {}",
                test_case.name
            );
            ...
            assert!(
                ...
                "Invalid trim limits for test case: {}",
                test_case.name
            );
        }
    }
