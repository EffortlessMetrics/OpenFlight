...
use crate::{FfbEngine, FfbError, Result, DeviceCapabilities, FfbMode};
use flight_hid::ofp1::{
    Ofp1Device, Ofp1Negotiator, Ofp1NegotiationResult, Ofp1HealthMonitor,
    TorqueCommandReport, HealthStatusReport, CommandFlags, StatusFlags,
    report_ids, utils, Ofp1Error,
};
...
impl TorquePathStabilityTracker {
    ...
    fn record_command(&mut self, torque_nm: f32) {
        ...
        self.recent_commands.retain(|(timestamp, _)| *timestamp > cutoff);
    ...
    fn is_stable(&self) -> bool {
        ...
        let values: Vec<f32> = self.recent_commands.iter().map(|(_, torque)| *torque).collect();
...
impl Ofp1FfbIntegration {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        info!("Starting OFP-1 FFB integration for device: {}", self.device.device_path());
    ...
    pub fn stop(&mut self) {
        ...
        info!("Stopping OFP-1 FFB integration for device: {}", self.device.device_path());
    ...
    fn negotiate_capabilities(&mut self) -> Result<()> {
        let capabilities = self.device.get_capabilities()
            .map_err(|e| FfbError::DeviceError { message: format!("Failed to get capabilities: {}", e) })?;
        
        let negotiator = Ofp1Negotiator::new();
        let result = negotiator.negotiate(&capabilities)
            .map_err(|e| FfbError::DeviceError { message: format!("Capability negotiation failed: {}", e) })?;
    ...
    fn integration_loop(
        ...
        while let Ok(command) = command_receiver.recv() {
            ...
            debug!("Processing OFP-1 torque command: {:?} (seq: {})", command, sequence);
    ...
    fn create_simulated_health_report(sequence: u16, command: &TorqueCommand) -> HealthStatusReport {
    ...
    pub fn send_torque_command(&mut self, torque_nm: f32, enable: bool, high_torque: bool, interlock_ok: bool) -> Result<()> {
        if !self.is_running {
            return Err(FfbError::DeviceError { message: "Integration not running".to_string() });
        }
        ...
        self.command_sender.try_send(command)
            .map_err(|e| FfbError::DeviceError { message: format!("Command queue full: {}", e) })?;
    ...
    pub fn trigger_emergency_stop(&mut self) -> Result<()> {
        ...
        self.command_sender.try_send(command)
            .map_err(|e| FfbError::DeviceError { message: format!("Emergency stop failed: {}", e) })?;
    ...
    pub fn process_health_updates(&mut self) -> Result<Vec<HealthStatusReport>> {
        ...
        while let Ok(health_update) = self.health_receiver.try_recv() {
            ...
            if let Err(e) = self.health_monitor.update_health(health_update.health_report.clone()) {
...
mod tests {
    use super::*;
    use crate::{FfbEngine, FfbConfig, FfbMode};
    ...
    fn test_torque_command_sending() {
        ...
        assert!(integration.send_torque_command(5.0, true, false, true).is_ok());
