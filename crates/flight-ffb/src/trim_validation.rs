...
impl TrimValidationSuite {
    ...
    fn test_ffb_no_torque_steps(&mut self) -> TrimValidationResult {
        ...
        let result = (|| -> Result<(), String> {
            ...
            if torque_steps > 0 {
                return Err(format!(
                    "Detected {} torque steps during setpoint change",
                    torque_steps
                ));
            }
        ...
        let result = (|| -> Result<(), String> {
            ...
            if max_overshoot > max_acceptable_overshoot {
                return Err(format!(
                    ...
                    max_overshoot,
                    max_acceptable_overshoot,
                    (max_overshoot / target) * 100.0
                ));
            }
    ...
    fn test_spring_freeze_behavior(&mut self) -> TrimValidationResult {
        ...
        let result = (|| -> Result<(), String> {
            ...
            if let Some(freeze_duration) = metrics.spring_freeze_duration {
                if freeze_duration < Duration::from_millis(50)
    ...
    fn test_mode_switching_stability(&mut self) -> TrimValidationResult {
        ...
        let result = (|| -> Result<(), String> {
            ...
            for cycle in 0..5 {
                ...
                controller
                    .apply_setpoint_change(ffb_change)
                ...
                controller
                    .apply_setpoint_change(spring_change)
        ...
        let result = (|| -> Result<(), String> {
            ...
            for (target_nm, should_fail) in extreme_cases {
                ...
                if should_fail && result.is_ok() {
                    ...
                    return Err(format!(
                        "Unexpected failure for setpoint {} Nm: {:?}",
                        target_nm, result
                    ));
                }
    ...
    pub fn test_replay_reproducibility(&mut self) -> TrimValidationResult {
        ...
        let result = (|| -> Result<(), String> {
            ...
            if self.config.verbose_logging {
                println!(
                    "Replay reproducibility: max difference = {} Nm",
                    max_difference
                );
            }
    ...
    pub fn generate_validation_report(&self, results: &[TrimValidationResult]) -> String {
        ...
        let ffb_tests: Vec<_> = results
            .iter()
            .filter(|r| r.name.starts_with("FFB"))
            .collect();
        let spring_tests: Vec<_> = results
            .iter()
            .filter(|r| r.name.starts_with("Spring"))
            .collect();
        let other_tests: Vec<_> = results
            .iter()
            .filter(|r| !r.name.starts_with("FFB") && !r.name.starts_with("Spring"))
            .collect();
