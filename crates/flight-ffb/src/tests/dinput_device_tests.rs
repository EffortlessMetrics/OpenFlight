...
mod tests {
    ...
    fn test_device_creation_platform_support() {
        ...
        #[cfg(windows)]
        {
            ...
            assert!(!dev.is_acquired(), "Device should not be acquired initially");
            assert_eq!(dev.get_effect_count(), 0, "No effects should exist initially");
        }
    ...
    fn test_device_initialization_stub() {
        ...
        assert!(!device.is_acquired(), "Device should not be acquired after init");
    }
    ...
    fn test_device_enumeration_stub() {
        ...
        assert!(device_list.is_empty(), "Stub enumeration should return empty list");
    }
    ...
    fn test_capability_query_defaults() {
        ...
        assert!(!caps.supports_raw_torque, "Should not support raw torque by default");
        assert_eq!(caps.max_torque_nm, 15.0, "Default max torque should be 15.0 Nm");
        assert_eq!(caps.min_period_us, 2000, "Default min period should be 2000 us (500 Hz)");
        assert!(!caps.has_health_stream, "Should not have health stream by default");
    ...
    fn test_device_acquisition_lifecycle() {
        ...
        assert!(!device.is_acquired(), "Device should not be acquired initially");
        
        // Acquire with null window handle (background mode)
        let result = device.acquire(0);
        assert!(result.is_ok(), "Acquisition should succeed");
        assert!(device.is_acquired(), "Device should be acquired after acquire()");
        ...
        // Release device
        device.unacquire();
        assert!(!device.is_acquired(), "Device should not be acquired after unacquire()");
    ...
    fn test_constant_force_effect_creation() {
        ...
        assert_eq!(pitch_handle.unwrap(), 0, "First effect should have handle 0");
        ...
        assert_eq!(roll_handle.unwrap(), 1, "Second effect should have handle 1");
    ...
    fn test_constant_force_magnitude_updates() {
        ...
        assert_eq!(device.get_last_torque_nm(), 15.0, "Should clamp to max_torque_nm");
    ...
    fn test_periodic_effect_parameters() {
        ...
        assert!(device.set_periodic_parameters(handle, 50.0, 1.5).is_ok(), "Should clamp magnitude to 1.0");
        assert!(device.set_periodic_parameters(handle, 50.0, -0.5).is_ok(), "Should clamp magnitude to 0.0");
    ...
    fn test_spring_effect_parameters() {
        ...
        assert!(device.set_spring_parameters(handle, 2.0, 0.5).is_ok(), "Should clamp center to 1.0");
        assert!(device.set_spring_parameters(handle, -2.0, 0.5).is_ok(), "Should clamp center to -1.0");
        
        // Test stiffness clamping
        assert!(device.set_spring_parameters(handle, 0.0, 1.5).is_ok(), "Should clamp stiffness to 1.0");
        assert!(device.set_spring_parameters(handle, 0.0, -0.5).is_ok(), "Should clamp stiffness to 0.0");
    }
    ...
    fn test_damper_effect_parameters() {
        ...
        assert!(device.set_damper_parameters(handle, 1.5).is_ok(), "Should clamp damping to 1.0");
        assert!(device.set_damper_parameters(handle, -0.5).is_ok(), "Should clamp damping to 0.0");
    }
    ...
    fn test_effect_start_stop() {
        ...
        assert!(device.start_effect(constant_h).is_ok(), "Should start constant force effect");
        assert!(device.stop_effect(constant_h).is_ok(), "Should stop constant force effect");
        
        // Test start/stop for periodic
        assert!(device.start_effect(periodic_h).is_ok(), "Should start periodic effect");
        assert!(device.stop_effect(periodic_h).is_ok(), "Should stop periodic effect");
        
        // Test multiple start/stop cycles
        assert!(device.start_effect(constant_h).is_ok());
        assert!(device.start_effect(constant_h).is_ok(), "Multiple starts should be safe");
        assert!(device.stop_effect(constant_h).is_ok());
        assert!(device.stop_effect(constant_h).is_ok(), "Multiple stops should be safe");
    }
    ...
    fn test_complete_device_lifecycle() {
        ...
        assert!(device.set_periodic_parameters(periodic_h, 20.0, 0.5).is_ok());
        ...
        assert!(device.set_periodic_parameters(periodic_h, 30.0, 0.8).is_ok());
    ...
    fn test_error_recovery() {
        ...
        assert!(device.set_constant_force(handle, 5.0).is_ok(), "Should recover from error");
        assert!(device.start_effect(handle).is_ok(), "Should still be able to start effect");
        assert!(device.stop_effect(handle).is_ok(), "Should still be able to stop effect");
    }
