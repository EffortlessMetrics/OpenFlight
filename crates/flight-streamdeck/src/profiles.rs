...
impl ProfileManager {
    ...
    pub fn load_sample_profiles(&mut self) -> Result<()> {
        ...
        self.profiles
            .insert(AircraftType::GA, serde_json::to_value(ga_profile)?);

        // Load Airbus profile
        let airbus_profile = self.create_airbus_profile()?;
        self.profiles
            .insert(AircraftType::Airbus, serde_json::to_value(airbus_profile)?);

        // Load Helicopter profile
        let helo_profile = self.create_helo_profile()?;
        self.profiles
            .insert(AircraftType::Helo, serde_json::to_value(helo_profile)?);
