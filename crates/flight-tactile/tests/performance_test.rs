...
fn test_tactile_processing_latency() {
    ...
    println!("Average processing time per snapshot: {}ns", avg_per_snapshot);
    
    // Verify processing time is reasonable (less than 10μs per snapshot)
    assert!(avg_per_snapshot < 10_000, "Processing too slow: {}ns per snapshot", avg_per_snapshot);
}
...
fn test_enable_disable_performance() {
    ...
    println!("Disabled time: {:?}, Enabled time: {:?}", disabled_time, enabled_time);
    
    // Enabled should not be significantly slower than disabled
    // Allow up to 2x overhead when enabled
    assert!(enabled_time < disabled_time * 2, "Enabled processing too much slower than disabled");
}
...
fn test_effect_processing_performance() {
    ...
    assert!(avg_per_test < 1_000, "Effect testing too slow: {}ns per test", avg_per_test);
}
...
fn test_config_update_performance() {
    ...
    assert!(avg_per_update < 100_000, "Config update too slow: {}ns per update", avg_per_update);
}
