...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    use flight_core::{
        ComponentType, PluginOverrunStats, SyntheticFault, WatchdogConfig, WatchdogEventType,
        WatchdogHealthSummary, WatchdogSystem,
    };
    use flight_hid::{EndpointType, HidAdapter, HidDeviceInfo};
...
#[cfg(feature = "flight-hid")]
fn demo_basic_watchdog_functionality(
    watchdog: &Arc<Mutex<WatchdogSystem>>,
) -> Result<(), Box<dyn std::error::Error>> {
    use flight_core::{ComponentType, WatchdogConfig, WatchdogSystem};
    use std::sync::{Arc, Mutex};
    use std::thread;
    ...
    let components = vec![
        (
            ComponentType::UsbEndpoint("demo_usb".to_string()),
            "USB Endpoint",
        ),
        (
            ComponentType::NativePlugin("demo_native".to_string()),
            "Native Plugin",
        ),
        (
            ComponentType::WasmPlugin("demo_wasm".to_string()),
            "WASM Plugin",
        ),
        (
            ComponentType::AxisNode("demo_axis".to_string()),
            "Axis Node",
        ),
    ];
    ...
    info!(
        "Initial health: {} total components, {} active",
...
#[cfg(feature = "flight-hid")]
fn demo_usb_endpoint_monitoring(
    hid_adapter: &mut HidAdapter,
    watchdog: &Arc<Mutex<WatchdogSystem>>,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Registered demo HID device: {}",
        device_info.product_name.as_ref().unwrap()
    );
    ...
    info!(
        "HID Adapter Stats: {} devices, {} endpoints, {} operations, {} bytes",
...
#[cfg(feature = "flight-hid")]
fn demo_plugin_overrun_detection(
    watchdog: &Arc<Mutex<WatchdogSystem>>,
) -> Result<(), Box<dyn std::error::Error>> {
    use flight_core::{ComponentType, WatchdogConfig, WatchdogSystem};
    use std::sync::{Arc, Mutex};
    ...
    for i in 0..5 {
        ...
        if let Some(event) = event {
            ...
            info!(
                "Plugin execution {} completed normally in {:?}",
                i + 1,
                execution_time
            );
        }
    ...
    for i in 0..3 {
        ...
        if let Some(event) = event {
            warn!(
                "Plugin overrun detected: {:?} - {}",
                event.event_type, event.context
            );
        }
    ...
    // Display plugin statistics
    {
        ...
        if let Some(stats) = wd.get_plugin_overrun_stats(plugin_id) {
            info!(
                "Plugin Stats: {} total overruns, {} recent executions, avg time: {:?}",
...
#[cfg(feature = "flight-hid")]
fn demo_quarantine_and_recovery(
    watchdog: &Arc<Mutex<WatchdogSystem>>,
) -> Result<(), Box<dyn std::error::Error>> {
    use flight_core::{ComponentType, WatchdogConfig, WatchdogSystem};
    use std::sync::{Arc, Mutex};
    ...
    for i in 1..=4 {
        ...
        info!(
            "Failure {}: {} - Quarantined: {}",
            i, event.context, is_quarantined
        );

        if is_quarantined {
            warn!(
                "Component {} has been quarantined!",
...
#[cfg(feature = "flight-hid")]
fn demo_synthetic_fault_injection(
    watchdog: &Arc<Mutex<WatchdogSystem>>,
) -> Result<(), Box<dyn std::error::Error>> {
    use flight_core::{
        ComponentType, SyntheticFault, WatchdogConfig, WatchdogEventType, WatchdogSystem,
    };
    use std::sync::{Arc, Mutex};
    use std::thread;
    use std::time::{Duration, Instant};
    ...
    for i in 0..5 {
        ...
        if !events.is_empty() {
            info!(
                "Processed {} synthetic fault events at iteration {}",
                events.len(),
                i + 1
            );
            for event in &events {
    ...
    // Display all recent events
    {
        ...
        let synthetic_count = recent_events
            .iter()
...
fn demo_health_monitoring(
    ...
    info!(
        "Recent NaN detections (5min): {}",
        summary.recent_nan_detections
    );
    info!(
        "Fault injection enabled: {}",
        summary.fault_injection_enabled
    );
    ...
    // Display detailed event history
    {
        ...
        info!(
            "=== Event History (last {} events) ===",
            all_events.len().min(10)
        );
