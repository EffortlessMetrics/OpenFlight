...
use flight_axis::{
    AxisEngine, AxisFrame, EngineConfig, PipelineBuilder, PipelineCompiler, UpdateResult,
    counters::{AllocationGuard, RuntimeCounters},
    nodes::{
        CurveNode, DeadzoneNode, DetentNode, DetentRole, DetentZone, MixerInput, MixerNode,
        SlewNode,
    },
};
#[cfg(feature = "integration")]
use flight_core::profile::{AxisConfig, CurvePoint, Profile};
#[cfg(feature = "integration")]
...
fn demo_performance_benchmarking() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  250Hz requirement: {} (need ≤4,000,000 ns)",
        if ns_per_frame <= 4_000_000 {
            "✓ PASS"
        } else {
            "✗ FAIL"
        }
