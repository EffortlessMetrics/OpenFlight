...
use flight_core::profile::{Profile, AxisConfig, AircraftId, DetentZone, CurvePoint, PofOverrides};
...
fn demo_profile_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(pitch_config) = non_monotonic.axes.get_mut("pitch") {
        pitch_config.curve = Some(vec![
            ...
            CurvePoint { input: 1.0, output: 0.3 }, // Non-monotonic!
        ]);
    }
...
fn demo_canonicalization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Canonical forms are identical: {}", canonical1 == canonical2);
...
fn demo_profile_merging() -> Result<(), Box<dyn std::error::Error>> {
    ...
    override_axes.insert("yaw".to_string(), AxisConfig {
...
fn demo_pof_overrides() -> Result<(), Box<dyn std::error::Error>> {
    ...
    approach_axes.insert("pitch".to_string(), AxisConfig {
        deadzone: None,
    ...
    pof_overrides.insert("approach".to_string(), PofOverrides {
