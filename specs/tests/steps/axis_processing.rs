...
async fn then_p99_latency(world: &mut FlightWorld, max_latency_ms: u64) {
    assert!(
        !world.latency_measurements.is_empty(),
...
async fn then_p99_jitter(world: &mut FlightWorld, max_jitter_ms: f64) {
    assert!(
        !world.jitter_measurements.is_empty(),
        "No jitter measurements recorded"
    );
...
fn simulate_axis_processing(num_axes: usize, rate_hz: u32, duration_secs: u64) -> Vec<f64> {
...
fn simulate_scheduler_jitter(rate_hz: u32, duration_secs: u64, warmup_secs: u64) -> Vec<f64> {
